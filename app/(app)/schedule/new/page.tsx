...
async function createAppointment(formData: FormData) {
  ...
  redirect(`/patients/${formData.get("patientId")?.toString() ?? ""}`);
}
...
export default async function NewAppointmentPage({
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Agendar sessão</h1>
        <p className="text-sm text-slate-500">
          Defina paciente, profissional e condições do atendimento.
        </p>
      </div>

      <form action={async (formData) => { "use server"; await upsertAppointmentAction(formData); redirect(`/patients/${formData.get("patientId")?.toString() ?? ""}`); }} className="space-y-6">
        <Card>
          <CardHeader>
          ...
          <div className="grid gap-3 px-6 pb-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                name="title"
                required
                placeholder="Consulta, profilaxia, procedimento"
              />
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
